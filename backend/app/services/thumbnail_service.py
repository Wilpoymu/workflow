...
def _build_thumbnail_analysis_prompt(script: str, mode: ThumbnailMode, variant_count: int) -> str:
    ...
    if mode == ThumbnailMode.AB_TESTING:
        variant_instruction = (
            ...
            '  - "hook_text": 3-4 word Spanish hook (emotional, different angle per variant)\n'
    ...
    fields = (
        ...
        '- "recommended_text": 3-4 word hook in SPANISH (this text will be generated by Flow as part of the image)\n'
        ...
        "The text must be in SPANISH and will be generated by the image AI as part of the visual."
    )
    if variant_instruction:
        fields += variant_instruction

    return (
        "You are a YouTube thumbnail designer. Given the Spanish script below, "
        ...
        f"{fields}\n"
        "RULES:\n"
        "- recommended_text: 3-4 words in SPANISH, high-impact emotional hook. This text will be "
        "generated by the image AI as part of the thumbnail — it must be in Spanish.\n"
        ...
        "- ALL text MUST be in Spanish with proper accents (á, é, í, ó, ú, ü, ñ, ¿, ¡).\n"
...
async def _generate_variants(
    ...
    for i, variant in enumerate(variants[:count]):
        ...
        prompt = f"{prompt}. The image must include the text \"{hook}\" in large, bold, stylized letters as the main visual element. Text must be in Spanish."
        try:
...
async def generate_thumbnail(
    ...
    if not background_path:
        ...
        # Include text in the prompt so Flow generates it as part of the image
        prompt = f"{prompt}. The image must include the text \"{text}\" in large, bold, stylized letters as the main visual element. Text must be in Spanish."
        fife_url = await bridge.dispatch_thumbnail(project_id, prompt)
        if fife_url:
    ...
    if request.mode == ThumbnailMode.AB_TESTING and ok:
        ...
        variants = await _generate_variants(
            project_id, project_dir, analysis, request.variant_count,
        )
